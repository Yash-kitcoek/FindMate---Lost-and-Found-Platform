...
router.post('/report-lost', isLoggedIn, upload.single('item-photo'), async (req, res) => {
    try {
        ...
        // ⭐ NEW: Trigger ML matching (non-blocking)
        mlMatchingService.findMatchesForLostItem(newLostItem)
            .catch(err => console.error('[ML] Matching failed:', err));
...
router.post('/report-found', isLoggedIn, upload.single('item-photo'), async (req, res) => {
    try {
        ...
        // ⭐ NEW: Trigger ML matching (non-blocking)
        mlMatchingService.findMatchesForFoundItem(newFoundItem)
            .catch(err => console.error('[ML] Matching failed:', err));
